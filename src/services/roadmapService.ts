import { supabase, Roadmap, UserProgress } from '../lib/supabase';
import { getFallbackRoadmap } from '../data/fallbackRoadmaps';

export async function fetchRoadmapByRole(role: string): Promise<Roadmap | null> {
  try {
...
function getFallbackRoadmapData(role: string): Roadmap | null {
...
export async function getUserProgress(roadmapId: string): Promise<number[]> {
  try {
...
export async function saveUserProgress(roadmapId: string, completedSteps: number[]): Promise<void> {
  try {
