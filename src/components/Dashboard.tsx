...
import { getUserProgress, saveUserProgress } from '../services/roadmapService';
...
export default function Dashboard({ roadmap, onReset }: DashboardProps) {
  ...
  const loadProgress = async () => {
    setIsLoading(true);
    const progress = await getUserProgress(roadmap.id);
  ...
  const handleToggleStep = async (stepNumber: number) => {
    ...
    await saveUserProgress(roadmap.id, newCompletedSteps);
  };
